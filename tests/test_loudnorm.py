...
class ApplyLoudnormTwoPassTests(unittest.TestCase):
    def test_two_pass_uses_measured_values_in_second_pass(self) -> None:
        ...
        # Second call should use measured values with linear=true
        second_call_cmd = mock_run.call_args_list[1][0][0]
        af_index = second_call_cmd.index("-af")
        filter_str = second_call_cmd[af_index + 1]
    ...
    def test_second_pass_includes_video_copy_and_audio_encoding(self) -> None:
        measure_result = _mock_subprocess_run_factory(stderr=SAMPLE_MEASUREMENT_JSON)
        apply_result = _mock_subprocess_run_factory()
        ...
        second_call_cmd = mock_run.call_args_list[1][0][0]
        self.assertIn("-c:v", second_call_cmd)
        cv_index = second_call_cmd.index("-c:v")
        self.assertEqual(second_call_cmd[cv_index + 1], "copy")
        self.assertIn("-c:a", second_call_cmd)
        ca_index = second_call_cmd.index("-c:a")
        self.assertEqual(second_call_cmd[ca_index + 1], "aac")
        self.assertIn("-movflags", second_call_cmd)
...
class MainTests(unittest.TestCase):
    ...
    def test_falls_back_to_preview_when_measurement_fails(self) -> None:
        ...
        with (
            ...
            mock_run.side_effect = [measure_result, preview_result]
            result = main()
    ...
    def test_preview_mode_skips_measurement(self) -> None:
        preview_result = _mock_subprocess_run_factory()
        with (
            mock.patch("media_tooling.loudnorm.subprocess.run", return_value=preview_result) as mock_run,
            mock.patch.object(
                sys, "argv", ["media-loudnorm", "input.mp4", "-o", "output.mp4", "--preview"]
            ),
            mock.patch("pathlib.Path.exists", return_value=True),
    ...
    def test_returns_1_on_ffmpeg_failure(self) -> None:
        ...
        with (
            ...
            mock_run.side_effect = [measure_result, apply_error]
            result = main()
